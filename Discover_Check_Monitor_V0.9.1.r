...
#  }
  #Add values for the API endpoint
  {
    url <- function() {
      tt <- tktoplevel()
      tkwm.title(tt, "API URL path")
      label <- tklabel(tt, text = "API URL path e.g. 'https://pure.<HEI>.ac.uk/ws/api/':")
      tkpack(label, padx = 10, pady = 5)
      api_url_var <- tclVar()
      entry <- tkentry(tt, textvariable = api_url_var, width = 50)
      tkpack(entry, padx = 10, pady = 5)
      on_ok <- function() {
        tkdestroy(tt)
      }
      ...
      tkpack(ok_button, pady = 10)
      tkwait.window(tt)
      return(tclvalue(api_url_var))
    }
    # Get the end date for the api using the pop-up box
    url <- url()
  }
  url<-paste0(url,"research-outputs/search")
    {
    api_key <- function() {
      tt <- tktoplevel()
